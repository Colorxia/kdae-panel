...
- 连接活动：以 dae 的 info 日志展示最近 24 小时内有界的连接建立流水，按目标、客户端、节点与出站组聚合并可直接筛选明细；客户端有有效 MAC 时跨 IP 合并。dae 当前持有的 TCP/UDP socket 与远端分布作为独立实时快照呈现，不读取内部 eBPF Map，也不伪造逐条存活状态、流量或速率；
