...
- 在官方 dae 发布与 kdae 分支 CI 构建之间安装或切换版本，安装前校验并在失败时自动回滚；机器上没有 dae 时可完成首次安装（默认关闭）；
...
- 面板自身的新版本提醒：读取本仓库最新发布并长时缓存，可用 `KDAE_PANEL_DISABLE_UPDATE_CHECK` 整体关闭；
- 面板一键自升级：校验 sha256、用新二进制自证可运行后再替换并重启自身，保留上一版供人工还原（独立开关，默认关闭）；
...
首次启动会枚举本机内网 IPv4，并在服务日志中为每个地址直接生成完整的一次性初始化链接，同时保留 `127.0.0.1` 作为本机入口。通过 systemd 安装时执行 `sudo journalctl -u kdae-panel -n 20 --no-pager`，选择当前设备能访问的 `setup_url` 即可。页面会自动完成授权，注册表单只需填写用户名和密码。创建管理员后初始化接口会永久关闭。
