...
| 方法 | 路径 | 说明 |
|---|---|---|
| `POST` | `/net/latency` | 绕过 dae，从面板主机直连目标做 TCP 握手延迟探测 |
...
单次最多 64 个目标，单目标总预算 4 秒，同一时刻最多 16 个并发目标（上限属于面板进程，多个并发请求共享）。域名先解析且不计入延迟；选定可达地址后最多握手三次，返回样本中位数。

`latencyMs` 是从发起 TCP 拨号到连接建立的耗时，不包含名称解析。该值反映面板主机到节点服务器入口的可达性，不等同于 dae 内部按 `tcp_check_url`/`udp_check_dns` 进行的健康检查，也不是 dae 选路时使用的延迟。

Linux 下每个探测 Socket 都设置 dae 从 v1 系列起保留的 `0x100` 绕行 Mark，避免 `wan_interface` 把连接重定向回本机控制平面而产生 `0.x ms` 假延迟。面板 systemd 单元已有设置 Mark 所需的 `CAP_NET_ADMIN`；权限不足时结果直接报错，不会静默回退到普通拨号。
