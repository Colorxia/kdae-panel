...
**信任边界**：这条命令等于信任本仓库和 GitHub。校验和与发布包放在同一个 Release 里，只能防传输损坏，防不了发布者本身。

### 手动下载安装

无法直连 GitHub，或不想直接执行远程脚本时：

1. 从 [Releases](https://github.com/tuoro/kdae-panel/releases) 下载 `kdae-panel_linux_<arch>.tar.gz` 和 `SHA256SUMS`；
...
```bash
gh attestation verify kdae-panel_linux_<arch>.tar.gz --repo tuoro/kdae-panel
```
