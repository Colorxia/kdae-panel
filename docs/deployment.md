...
首次启动会在服务日志里生成一次性初始化链接：

```bash
sudo journalctl -u kdae-panel -n 20 --no-pager
```

面板会枚举本机 RFC1918 内网 IPv4，为每个地址分别输出完整的 `setup_url`，并保留一条 `127.0.0.1` 本机链接。多网卡机器可能出现多条，选择当前设备能访问的那一条即可。页面会自动完成授权，注册表单只需填写用户名和密码。创建管理员后初始化接口永久关闭。
