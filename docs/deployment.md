...
版本页也可以卸载 dae。该操作会立即停止代理服务，删除面板管理的 dae 可执行文件、标准路径下的 systemd 单元和版本回滚记录，但保留 `/etc/dae/config.dae`、订阅与 geo 数据，之后可直接在同一页面重新安装。为避免误删包管理器或用户手工维护的程序，没有面板安装记录、二进制摘要已经漂移，或服务单元不在标准路径时，自动卸载会被拒绝。
